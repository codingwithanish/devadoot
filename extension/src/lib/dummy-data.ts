...
export const DUMMY_MARKETPLACE_AGENTS: MarketplaceAgent[] = [
  // Shopping Category
  {
    ...
    type: 'marketplace',
    category: 'Shopping',
    description: 'Monitors prices across different sites and alerts you to better deals',
...
export const DUMMY_AGENTS: AgentConfig[] = [
  {
    ...
    monitoring: 'Both',
    ruleNL: 'Activate when user visits Amazon page containing Ecovacs vacuum cleaner products',
    welcomeMessage: 'I am your Ecovacs Vacuum Cleaner Support Agent. Do you want to know more about our product?',
    collectors: {
      har: true,
      console: true,
      cookies: false,
  ...
  },
  {
    ...
    welcomeMessage: 'I found this product! Would you like me to check if there are better prices elsewhere?',
    collectors: {
      har: false,
      console: false,
      cookies: false,
  ...
  },
  {
    ...
    welcomeMessage: 'Hold on! Let me find the best promo codes for your purchase.',
    collectors: {
      har: false,
      console: false,
      cookies: false,
  ...
  },
  {
    ...
    welcomeMessage: 'Hi! I noticed you might need some help. How can I assist you today?',
    collectors: {
      har: true,
      console: true,
      cookies: false,
...
export function getDummyAgentMatches(url: string): AgentMatch[] {
  ...
  for (const agent of DUMMY_AGENTS) {
    ...
    console.log(`[DUMMY MODE] Agent "${agent.name}": hostname="${hostname}", sites=${JSON.stringify(agent.sites)}, siteMatch=${siteMatch}`);
    ...
    for (const pattern of agent.urlPatterns) {
      try {
        ...
        if (regex.test(url)) {
          patternMatch = true;
          console.log(`[DUMMY MODE] Agent "${agent.name}": URL pattern "${pattern}" MATCHED!`);
          break;
        }
      } catch (e) {
    ...
    console.log(`[DUMMY MODE] Agent "${agent.name}": patternMatch=${patternMatch}, urlPatterns=${JSON.stringify(agent.urlPatterns)}`);

    if (siteMatch && patternMatch) {
      console.log(`[DUMMY MODE] ✓ Agent "${agent.name}" MATCHED for URL: ${url}`);
      ...
      matches.push({
        ...
        agentChatMeta: {
          type: agent.source,
          endpoint: agent.source === 'marketplace'
  ...
  console.log(`[DUMMY MODE] Total matches found: ${matches.length}`, matches.map(m => m.name));
  return matches;
}
